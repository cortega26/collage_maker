...
class CollageWidget(QWidget):
    ...
    def merge_cells(self, start_row: int, start_col: int, row_span: int, col_span: int):
        ...
        for r in range(start_row, start_row + row_span):
            for c in range(start_col, start_col + col_span):
                ...
                if cell and cell.pixmap:
                    cells_with_content.append(cell)

        # Warn if merging will delete images
        if cells_with_content:
            logging.warning(f"Merging will remove images from {len(cells_with_content)} cells")
            
        # Store cells to remove
        cells_to_remove = []
        ...
        # Update the grid layout with the merged cell
    ...
    def is_valid_merge(self, start_row: int, start_col: int, row_span: int, col_span: int) -> bool:
        ...
        # Check if any cell in the range is part of an existing merge
        for r in range(start_row, start_row + row_span):
            for c in range(start_col, start_col + col_span):
                for (mr, mc), (mrs, mcs) in self.merged_cells.items():
                    if (mr <= r < mr + mrs) and (mc <= c < mc + mcs):
                        logging.warning(f"Cell at ({r},{c}) is already part of merge at ({mr},{mc})")
                        return False
                
                # Verify cell exists at this position
                if not self.get_cell_at_position(r, c):
                    logging.warning(f"No cell found at position ({r},{c})")
                    return False
    ...
    def split_merged_cell(self, row: int, col: int):
        ...
        # Find the merge entry that contains this cell
        merge_key = None
        for (mr, mc), (mrs, mcs) in self.merged_cells.items():
        ...
        start_row, start_col = merge_key
        row_span, col_span = self.merged_cells[merge_key]
        ...
        # Remove the merged cell from the layout
        self.grid_layout.removeWidget(target_cell)
        
        # Reset target cell properties
        target_cell.row_span = 1
        target_cell.col_span = 1
        target_cell.setFixedSize(self.cell_size, self.cell_size)
        
        # Add it back to its original position
        self.grid_layout.addWidget(target_cell, start_row, start_col, 1, 1)
        
        # Create new cells for the previously merged area
        for r in range(start_row, start_row + row_span):
            for c in range(start_col, start_col + col_span):
                if r == start_row and c == start_col:
                    continue  # Skip the original cell position
        ...
        # Remove the merge information
        del self.merged_cells[merge_key]
        
        logging.info(f"Split merged cell at ({start_row},{start_col})")
        self.update()
    ...
    def get_cell_position(self, cell: CollageCell) -> tuple[int, int]:
        """Get the grid position (row, col) of a cell."""
        item = self.grid_layout.indexOf(cell)
        if item == -1:
            return None
        pos = self.grid_layout.getItemPosition(item)
        if pos:
            return pos[0], pos[1]  # row, col
        return None

    def get_cell_at_position(self, row: int, col: int) -> CollageCell:
    ...
    def merge_selected_cells(self):
        # Find selected cells and their positions
        selected_cells = []
        selected_positions = []
        ...
        min_row = min(pos[0] for pos in selected_positions)
        min_col = min(pos[1] for pos in selected_positions)
        max_row = max(pos[0] for pos in selected_positions)
        max_col = max(pos[1] for pos in selected_positions)

        # Check if selection forms a valid merge area (cells must be adjacent)
        selected_set = set(selected_positions)
        
        # For a valid merge, cells must be adjacent in either row or column direction
        is_vertical = all(any((r, min_col) in selected_set for r in range(min_row, max_row + 1))
                         for c in range(min_col, max_col + 1))
        is_horizontal = all(any((min_row, c) in selected_set for c in range(min_col, max_col + 1))
                          for r in range(min_row, max_row + 1))
        
        if not (is_vertical or is_horizontal):
            logging.warning("Selected cells must be adjacent")
            return
        ...
        # Clear selection before merging
        for cell in selected_cells:
            cell.selected = False
            cell.update()

        # Attempt to merge
        self.merge_cells(min_row, min_col, row_span, col_span)
...
class MainWindow(QMainWindow):
    ...
    def create_controls_panel(self):
        ...
        row1.addWidget(update_button)
        row1.addWidget(save_button)
        row1.addWidget(merge_button)
        row1.addWidget(split_button)
        
        # Imagen quality controls
        quality_layout = QHBoxLayout()
        quality_label = QLabel("Calidad de imagen:")
        
        self.transform_combo = QComboBox()
        self.transform_combo.addItem("Alta calidad", Qt.SmoothTransformation)
        self.transform_combo.addItem("Balanceada", Qt.SmoothTransformation)
        self.transform_combo.addItem("Rápida", Qt.FastTransformation)
        self.transform_combo.setCurrentIndex(0)
        self.transform_combo.currentIndexChanged.connect(self.update_image_quality)
        
        self.aspect_combo = QComboBox()
        self.aspect_combo.addItem("Mantener proporción", Qt.KeepAspectRatio)
        self.aspect_combo.addItem("Estirar para llenar", Qt.IgnoreAspectRatio)
        self.aspect_combo.addItem("Encajar dentro", Qt.KeepAspectRatioByExpanding)
        self.aspect_combo.setCurrentIndex(0)
        self.aspect_combo.currentIndexChanged.connect(self.update_image_quality)
        
        # Agregar controles de calidad de imagen
        ...
        # Agregar controles de caption al layout de caption
        ...
        controls_vertical.addLayout(caption_controls_layout)
        ...
        row1.addWidget(batch_import_button)
    ...
    def split_selected_cell(self):
        # Find the selected merged cell
        for cell in self.collage.cells:
            if cell.selected:
                pos = self.collage.get_cell_position(cell)
                if pos:
                    row, col = pos
                    if self.collage.is_cell_merged(row, col):
    ...
    def get_collage_state(self) -> dict:
        ...
        state = {
            ...
            'cells': [],
            'merged_cells': self.collage.merged_cells,
            'settings': {
    ...
    def restore_collage_state(self, state: dict):
        ...
        try:
            ...
            self.collage.merged_cells = state['merged_cells']
            for (row, col), (row_span, col_span) in state['merged_cells'].items():
